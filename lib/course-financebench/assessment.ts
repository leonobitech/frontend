// ─── Financial RAG Evaluation Suite — Schema + scoring del assessment ───
//
...
//
// Diferencias respecto a `lib/course/assessment.ts` (rust-embedded):
//   - `courseSlug` literal apunta a `financial-rag-eval-from-zero`.
//   - `sourceStep` sin `.max(9)` — el curso tiene ~19 lessons.
//   - `questions` permite array vacío (`.min(0)`) — al bootstrap el
//     assessment empieza vacío y se llena por Trigger A. La UI maneja
//     el caso 0-questions con un mensaje "Coming soon".
...
export const assessmentSchema = z.object({
  courseSlug: z.literal("financial-rag-eval-from-zero"),
  title: z.string().min(1),
  description: z.string().min(1),
  /** Threshold 0-100 — 70 = 70% de respuestas correctas. */
  passingScore: z.number().int().min(0).max(100),
  shuffle: z.boolean(),
  /** Bootstrap permite array vacío — la UI muestra "Coming soon" hasta el primer Trigger A. */
 ...
 * attempts pero distinto del resto.
 */
