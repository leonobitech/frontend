...
const transformPodcast = (podcast: PodcastFromAPI): PodcastEpisode => ({
  ...
  cover: "/podcast-02.jpeg", // Default cover
...
export default function PodcastPlayer() {
  ...
  const fetchPodcasts = useCallback(async () => {
    try {
      ...
      toast.error("Error al cargar los podcasts");
    } finally {
      setIsLoading(false);
    }
  }, [currentEpisode]);
  ...
  const shareOnTwitter = () => {
    ...
    const text = `Listen to "${currentEpisode.title}" by ${currentEpisode.artist} on Leonobitech Podcasts!`;
  ...
  const changeEpisode = (direction: "next" | "previous") => {
    if (!currentEpisode || podcasts.length === 0) return;
    ...
    if (direction === "next") {
      newIndex = (currentIndex + 1) % podcasts.length;
    } else {
      newIndex = (currentIndex - 1 + podcasts.length) % podcasts.length;
    }
    ...
    return (
      <div className="container mx-auto px-4 pt-12 pb-8">
        ...
        <p className="text-center text-gray-600 dark:text-gray-300 mb-8 max-w-2xl mx-auto">
          A podcast where we dive into the most exciting topics in technology,
          AI, neuroscience, entrepreneurship, and self-improvement.
        </p>
          ...
          <div className="flex justify-center mb-8">
            <Sheet open={isUploadOpen} onOpenChange={setIsUploadOpen}>
              <SheetTrigger asChild>
                <Button className="bg-linear-to-r from-blue-600 to-blue-800 hover:from-blue-500 hover:to-blue-700 text-white shadow-lg">
                  <Plus className="h-4 w-4 mr-2" />
                  Agregar Primer Podcast
                </Button>
              </SheetTrigger>
              <SheetContent
                side="right"
                className="w-full sm:max-w-lg bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 border-white/10 text-white overflow-y-auto"
              >
                <SheetHeader className="mb-6">
                  ...
                  <SheetDescription className="text-white/60">
                    Sube un video y completa la informacion del episodio
                  </SheetDescription>
                </SheetHeader>
                <VideoUploader
        ...
        <div className="text-center text-gray-500 py-16">
          <p>No hay podcasts disponibles todavia.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 pt-12 pb-8">
      ...
      {isAdmin && (
        <div className="flex justify-center mb-8">
          <Sheet open={isUploadOpen} onOpenChange={setIsUploadOpen}>
            <SheetTrigger asChild>
              <Button className="bg-linear-to-r from-blue-600 to-blue-800 hover:from-blue-500 hover:to-blue-700 text-white shadow-lg hover:shadow-blue-500/25 transition-all duration-300">
                <Plus className="h-4 w-4 mr-2" />
                Agregar Podcast
              </Button>
            </SheetTrigger>
      ...
      <Card
        className={`max-w-xl mx-auto mb-12 border bg-linear-to-br from-blue-900/40 via-slate-900/30 to-blue-950/40 backdrop-blur-2xl text-white overflow-hidden ring-2 transition-all duration-300 ${
          isPlaying
        ...
        {podcasts.map((episode) => (
            ...
            className={`group cursor-pointer transition-all duration-500 hover:scale-[1.02] border ring-2 will-change-transform
                ${
                  currentEpisode.id === episode.id && isPlaying
                    ? "bg-linear-to-br from-blue-500/30 via-cyan-500/20 to-blue-600/30 border-blue-400/50 ring-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.5),0_0_30px_rgba(59,130,246,0.3),0_0_45px_rgba(59,130,246,0.1)]"
                    : currentEpisode.id === episode.id
                    ? "bg-linear-to-br from-blue-500/30 via-cyan-500/20 to-blue-600/30 border-blue-400/50 ring-white/10 shadow-blue-500/30"
          ...
          >
            <CardContent className="px-5 md:py-6 py-6 flex items-center justify-between space-x-4 [backdrop-filter:none]">
              <div
                className={`relative w-20 h-20 shrink-0 rounded-xl overflow-hidden ring-2 transition-all duration-300 ${
                  currentEpisode.id === episode.id && isPlaying
              ...
              <div className="grow min-w-0 space-y-1">
                ...
                <p className="text-xs sm:text-sm text-gray-400 line-clamp-1 group-hover:text-gray-300 transition-colors duration-300">
                  {episode.description || `By ${episode.artist}`}
                </p>
                <div className="flex items-center text-xs text-gray-500 group-hover:text-cyan-300 transition-colors duration-300">
                  ...
                  <span>{formatDurationDisplay(episode.duration)}</span>
                </div>
              </div>
