...
export async function POST(request: Request) {
  try {
    ...
    const token = jwt.sign(
      ...
      {
        algorithm: "HS256",
        audience: "lab-webrtc-05-audio", // aud específico del lab-05
        issuer: "lab-05", // iss específico del lab-05
        subject: String(sub),
      }
    );
    ...
    const backendRes = await axios.post(
      `${AXUM_API_ORIGIN}/webrtc/lab/05/offer`,
