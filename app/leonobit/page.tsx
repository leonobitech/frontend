...
/* ---------- helper: status driver (0=closed,1=connecting,2=open) ---------- */
...
function MolecularCore({ status }: { status: UIStatus }) {
  ...
  const data = useMemo(() => {
    ...
    // conexiones probabilísticas → curvadas
  ...
  useFrame(({ clock }) => {
    ...
    // Nodos (meshes hijos primero)
    let idx = 0;
    for (const child of g.children) {
      // solo afectar a los Mesh (los <primitive> de líneas no son Mesh)
      // @ts-expect-error narrow en runtime
      if (child.isMesh) {
    ...
    // Filamentos: opacidad respira con estado
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* FILAMENTOS (THREE.Line via <primitive/>) */}
